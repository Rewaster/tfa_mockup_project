...
def _get_fernet_hash_key() -> Fernet:
    ...
    FERNET_KEY_TFA_TOKEN deploy variable

    Returns:
        Fernet: _description_
    """
    return Fernet(key=bytes(settings.FERNET_KEY_TFA_TOKEN, ENCODING))
...
def get_decoded_two_factor_auth_key(value: str) -> str:
    """Decode a TFA authentication key"""
...
def _get_fake_otp_token(nr_digits: int = settings.TFA_TOKEN_LENGTH) -> str:
    ...
    Returns:
        str
    """
    return "".join(str(random.randint(0, 9)) for _ in range(nr_digits))


def get_fake_otp_tokens(
    nr_digits: int = settings.TFA_TOKEN_LENGTH, nr_tokens: int = settings.TFA_BACKUP_TOKENS_NR
) -> Iterator[str]:
    """Generate several fake OTP tokens and return a random one"""
...
def verify_backup_token(backup_tokens: list[BackupToken], tfa_backup_token: str) -> BackupToken | None:
...
def send_tfa_token(user: User, device_type: DeviceTypeEnum) -> None:
    ...
    if device_type == DeviceTypeEnum.EMAIL:
        ...
        send_mail_totp_token(user=user, token=current_totp)
...
def qr_code_from_key(encoded_key: str, user_email: str) -> bytes:
    ...
    qrcode_key = pyotp.TOTP(decoded_key, interval=settings.TOTP_TOKEN_TOLERANCE).provisioning_uri(
        user_email, issuer_name=settings.TOTP_ISSUER_NAME
    )
